...
export default function EcosystemDirectory({ kind }) {
  ...
  return <><SiteHeader /><main className="min-h-screen bg-edvanta-bg"><section className="border-b border-slate-200 bg-white"><div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16"><div className="flex max-w-4xl items-start gap-4"><span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-teal-50 text-teal-700"><Icon className="h-6 w-6" /></span><div><p className="text-sm font-bold uppercase text-teal-700">{config.eyebrow}</p><h1 className="mt-3 text-4xl font-bold leading-tight text-edvanta-deep sm:text-5xl">{config.title}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">{config.description}</p></div></div></div></section>{filters.length > 0 && <div className="border-b border-slate-200 bg-edvanta-bg"><div className="mx-auto flex max-w-7xl gap-2 overflow-x-auto px-4 py-4 sm:px-6 lg:px-8">{filters.map(([value,label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={`min-h-10 shrink-0 rounded-full border px-4 text-sm font-bold ${filter === value ? 'border-teal-600 bg-white text-teal-800' : 'border-transparent text-slate-600'}`}>{label}</button>)}</div></div>}<section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">{loading ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{Array.from({length:6}).map((_,i)=><div key={i} className="h-64 animate-pulse rounded-lg border border-slate-200 bg-white" />)}</div> : error ? <div className="rounded-lg border border-slate-200 bg-white px-6 py-16 text-center"><RefreshCw className="mx-auto h-9 w-9 text-teal-700" /><h2 className="mt-4 text-xl font-bold text-edvanta-deep">No pudimos cargar el directorio</h2><p className="mt-2 text-sm text-slate-600">{error}</p><button type="button" onClick={() => setReload(value => value + 1)} className="mt-5 min-h-11 rounded-lg bg-edvanta-blue px-5 text-sm font-bold text-white">Reintentar</button></div> : visibleItems.length ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{visibleItems.map(item => <DirectoryCard key={item.id || item.slug} kind={kind} item={item} />)}</div> : <div className="rounded-lg border border-slate-200 bg-white px-6 py-16 text-center"><Icon className="mx-auto h-10 w-10 text-teal-700" /><h2 className="mt-5 text-2xl font-bold text-edvanta-deep">{config.emptyTitle}</h2><p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-600">{config.emptyText}</p><Link to={config.action[1]} className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-lg bg-edvanta-blue px-5 text-sm font-bold text-white">{config.action[0]} <ArrowRight className="h-4 w-4" /></Link></div>}</section></main><SiteFooter /></>;
}
