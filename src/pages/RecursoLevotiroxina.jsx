...
// (ASCII para evitar problemas de codificación con el carácter ★).
...
export default function RecursoLevotiroxina() {
  ...
  const [rState, setRState] = useState('idle'); // idle | sending | error
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(`/api/article-comments/${REVIEW_SLUG}`))
      ...
      .then(d => {
        if (!alive) return;
        const list = (d.comments || []).filter(c => !c.parent_id).map(parseComment);
        setReviews(list.reverse()); // más recientes primero
      })
      .catch(() => { /* sin backend (dev) → queda vacío */ });
    return () => { alive = false; };
  }, []);
  ...
  const submitReview = async (e) => {
    ...
    try {
      const res = await fetch(apiUrl('/api/article-comments'), {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          article_slug: REVIEW_SLUG,
          user_name: rName.trim(),
          body: encodeBody(rStars, rText.trim()),
          parent_id: null,
        }),
      });
      ...
      setRState('error');
      setRError('No pudimos publicar tu reseña. Intenta de nuevo en un momento.');
    }
  };
    ...
    <div className="min-h-screen bg-sand-50 font-sans">
      ...
      <section id="resenas" className="max-w-3xl mx-auto px-4 sm:px-6 py-14">
        ...
        <div className="space-y-3">
          ...
          {reviews.map((r) => (
            <div key={r.id} className="bg-white rounded-2xl border border-sand-100 p-4 shadow-sm">
